...
5. The QBR draft renders in the feed with a scroll panel — read it in the browser
6. A `QBR_GPUs_Q4_2024.docx` Word file auto-downloads to your computer the moment the draft is ready — no button, no scrolling required

**Cost:** ~$0.01 per full QBR run (claude-haiku-4-5, 8 tool calls + narrative generation)

**Note:** The Word doc is built entirely in the browser with no external libraries — pure JavaScript ZIP construction. Works offline once the page has loaded.
